...
| `scripts/plan.sh` | **What the NAS actually runs.** The host-side DSM Task Scheduler entry, hourly at :05: one `plan-now.sh` pass in the container, then `capture_weather.py` non-fatally beside it. Lives outside the image on purpose — it is what starts the container, so it cannot be inside it. |
