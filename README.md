...
| `report_day.py` | **Day-after report.** Scores a past day's stored plans against what actually happened, in three separate sections: money at the prices that applied, outcomes (SoC and grid), and forecast error for PV and load. Sections 1 and 2 judge the day against one committed plan - the latest run that already covered the day's first interval - and section 4 refuses to publish a plan whose energy does not add up. The in-force chain that used to be scored is printed beside it as a diagnostic: every run restarts from the measured SoC, so chaining runs spends the same battery several times and reads high or low with no reliable sign. `--write` stores the comparison back as measurement `plan_score`. |
| `report_window.py` | **Plan-vs-actual, interval by interval.** Where `report_day.py` scores a whole day, this prints a chosen window side by side — planned against measured SoC, grid and PV — for answering *why* the two diverged when a day's score looks wrong. |
...
| `fit_load_profile.py` | **Load forecast check.** Reads the stored `load_forecast_wh` against measured load over N days (default 30) and reports the overall level, the ratio per hour of day, and the same split weekday against weekend. There is no calibration constant to fit here — the answer to a bias is a change to how `influx_source.hourlyAvgProfileWh()` builds the profile. |
| `capture_weather.py` | **Weather, forward.** Stores the current hourly forecast as measurement `weather_forecast`, tagged `weather_run` the way plans are tagged `plan_run`. Runs beside the planner on its hourly schedule, non-fatally. The perishable half: what a forecast said at the time cannot be reconstructed once it is superseded. |
| `backfill_weather.py` | **Weather, backward.** Fills in past hourly weather as `weather_observed`. Idempotent — no run tag, so a re-run overwrites rather than duplicates. Unhurried by design: observations can be re-fetched from Open-Meteo for ever, so nothing is lost by running this a year from now. |
