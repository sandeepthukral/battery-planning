...
| `capture_weather.py` | **Weather, forward.** Stores the current hourly forecast as measurement `weather_forecast`, tagged `weather_run` the way plans are tagged `plan_run`. Runs beside the planner on its three-hourly schedule, non-fatally. The perishable half: what a forecast said at the time cannot be reconstructed once it is superseded. |
